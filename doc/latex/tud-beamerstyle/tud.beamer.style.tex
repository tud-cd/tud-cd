...
\maketitle
\mode<presentation>{\setbeamertemplate{page number in footline}[page][text and total]}
\frame{\frametitle{Inhalt}\tableofcontents}
\mode<presentation>{\setbeamertemplate{tud background}[image/shaded/tikzoverlay]{Posterflur.jpg}{0.6}{%
  \node[anchor=south east] at (\tudbeamerbackgroundwidth,0) {\footnotesize Foto: Tobias Schlemmer};
}}
\section{Geschichte}
\mode<presentation>{\setbeamertemplate{page number in footline}[page][text and filetotal]}
...
\mode<presentation>{\setbeamertemplate{page number in footline}[page][text]}
...
\begin{frame}[allowframebreaks]\frametitle{2010 – Evaluierung der Fachrichtung Mathematik}
  ...
  verteilt.%
  \mode<presentation>{\setbeamertemplate{page number in footline}[page][only]}
...
\begin{frame}[allowframebreaks]
  ...
  \end{block}
\end{frame}
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text and filetotal]}
\begin{frame}[allowframebreaks]
...
\end{frame}
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text and total and overlay]}
\begin{frame}
...
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text and filetotal and overlay]}
\mode<presentation>{\setbeamertemplate{tud background}[image/shaded/overlay]{Schreibwerkzeug.jpg}{0.6}{%
  \begin{tikzpicture}%
    \useasboundingbox (0,0) -- (\tudbeamerbackgroundwidth,\tudbeamerbackgroundheight);
    \node[anchor=north east] at (\tudbeamerbackgroundwidth,\tudbeamerbackgroundheight) {\footnotesize Arrangement/Foto: Tobias Schlemmer};
  \end{tikzpicture}%
}}
\section{Benutzung}
...
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text]}
\begin{frame}[allowframebreaks]{Optionen}
  \begin{description}
    \item[heavyfont] Stärkere Schriften
    \item[nodin] Lade keine {\csname dinfamily\endcsname DIN bold}% nur wenn dinfamily definiert ist.
...
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text and overlay]}
\makeatletter
\let\tstempboxa\@tempboxa
\makeatother
\mode<presentation>{\setbeamertemplate{tud background}[image/shaded/pgfoverlay]{Baustelle.jpg}{0.6}{
  \setbox\tstempboxa\hbox{%
    \begin{pgfinterruptpicture}%
      \begin{tikzpicture}%
        \useasboundingbox (0,0) -- (\tudbeamerbackgroundwidth,\tudbeamerbackgroundheight);
        \node[anchor=south east] at (\tudbeamerbackgroundwidth,0) {\footnotesize Neue Mensa Dresden};
      \end{tikzpicture}%
...
\mode<presentation>{\setbeamertemplate{page number in footline}[frame][text and filetotal]}
\mode<presentation>{\setbeamertemplate{tud background}[image/pgfoverlay]{Ausfahrt.jpg}{0.6}{
  ...
  \setbox\tstempboxa\hbox{%
    \begin{pgfinterruptpicture}%
      \begin{tikzpicture}%
        \useasboundingbox (0,0) -- (\tudbeamerbackgroundwidth,\tudbeamerbackgroundheight);
        \node[anchor=south east] at (\tudbeamerbackgroundwidth,0) {\footnotesize Gute Fahrt!};
      \end{tikzpicture}%
    \end{pgfinterruptpicture}%
  }%
  \pgfqboxsynced\tstempboxa
...
\begin{frame}
  ...
  \[
    \alpha αβγδεζητικλμνξοπρσυφχψω⇐⇔⇒↦ \Alpha\Beta\Gamma\Delta\Epsilon\Zeta\Eta\Tau\Iota\Kappa\Lambda\Mu\Nu\Xi\Omicron\Pi\Rho\Sigma\Upsilon\Phi\Chi\Psi\Omega
  \]
\end{frame}
